...
compiler.  Provide a clear justification for your response.
...
\item ({\bf 10 Points}) The {\tt Queue} is an abstract data type that
  ...
  \item ({\bf 4 Points}) What is the worst-case space complexity of an
    ...
    should clearly state why your selected space complexities
...
\begin{enumerate}
    ...
    \item ({\bf 2 Points}) What are the similarities and differences between a {\tt Queue} and a {\tt DoubleEndedQueue}?
    Your response to this question should explain and then compare and constrast the key methods provided by each of these
    data structures.
    ...
    \item ({\bf 2 Points}) Suppose that you have correctly implemented an extendable array using a fixed-size
    ``backing'' array. What is the worst-case time complexity of the {\tt get(i)} and {\tt set(i,e)} methods? Why did
    you pick these time complexities?

    \item ({\bf 4 Points}) The {\tt java.util.Vector} has a constructor that accepts two parameters in this manner: {\tt
    Vector(int initialCapacity, int capacityIncrement)}.  Explain the purpose of each of these parameters and discuss how
    it will impact the performance of the {\tt Vector} abstract data type.
